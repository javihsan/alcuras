
	<%@include file="CabeceraSalud.jsp"%>
	
	        <div class="textHome health inst">
	        	<div class="break" style="float:left; margin-left:150px;">
	        		<img src="/web/img/corpo/001_AlcurasCorpo.jpg" width="310" height="401" class="" />
	        	</div>
	        	<div class="break" style="float:left; margin-right:10px; margin-top:60px;">
	        		<img src="/web/img/corpo/003_AlcurasCorpo.jpg" width="310" height="261" class="" />
	        	</div>
	        	<div class="" style="float:left; margin-right:10px; margin-top:30px;">
	        		<img src="/web/img/corpo/005_AlcurasCorpo.jpg" width="310" height="441" class="" />
	        	</div>
	        	<div class="" style="float:left; margin-right:10px; margin-top:-50px;">
	        		<img src="/web/img/corpo/007_AlcurasCorpo.jpg" width="310" height="401" class="" />
	        	</div>
	         
	        </div>
	    </div>		
			
	
	<%@include file="Pie.jsp"%>